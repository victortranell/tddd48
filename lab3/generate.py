...
parser.add_option('-b', '--ground_robots',metavar='NUM', dest='ground_robot',action='store',type=int, help='the number of Ground Robots')
...
if (options.uavs == None or options.ground_robot == None or options.carriers == None or
...
print "Ground Robots\t\t", options.ground_robot
...
for x in range(options.ground_robot):
	ground_robot.append("ground_robot"+str(x+1))
...
problem_name ="uav_problem_"+str(options.uavs)+"_"+str(options.ground_robot)+"_"+str(options.carriers)+\
...
for x in ground_robot:
	f.write("\t" + x + " - ground_robot\n")
...
for x in range(0,4):
	f.write("\t(next n" + str(x) + " n" + str(x+1) + ")\n")
...
for l1 in location + ["depot"]:
	for l2 in location + ["depot"]:
		...
		if(l1 == l2):
			...
			cost = 500
		f.write("\t(= (fly-duration %s %s) %s)\n"%(l1, l2, cost))
		f.write("\t(= (move-duration %s %s) %s)\n"%(l1, l2, 2*cost))	
